...
- Diagnostics 顶部显示 dev build version / branch / commit。
...
- 无真实用户文件写入。
- 无 lockfile 修改。
- 无浏览器执行 Shell；Git 信息只在 Vite 启动时由 dev server 注入。
...
dev:open started Local Core / Web and opened app-mode browser.
```
