...
export function createChildScopeFromSelection(
  ...
  const container: CanvasNode = {
    ...
    entityKind: input.kind === 'workflow' ? 'workflow' : input.kind === 'context' ? 'context' : 'collection',
    contextOnly: true,
  }
...
/**
 * R3.1-A: create only a durable aggregate identity + its canvas proxy.
 * No member View is cloned into the Scope. Collection / Context / Workflow
 * membership is stored by Presentation and may reference Project entities
 * regardless of their physical navigation Scope.
 */
export function createAggregateScopeEntity(input: {
  parentScopeId: string
  label: string
  kind: 'collection' | 'context' | 'workflow'
  memberCount?: number
  containerPosition: { x: number; y: number }
  createId: (prefix: string) => string
}): { scope: CanvasScope; container: CanvasNode } {
  const scopeId = input.createId('scope')
  const containerId = input.createId(input.kind)
  const count = input.memberCount ?? 0
  const noun = input.kind === 'context' ? 'Context' : input.kind === 'workflow' ? 'Workflow' : '集合'
  const container: CanvasNode = {
    id: containerId,
    artifactId: input.createId(`artifact-${input.kind}`),
    kind: 'context',
    title: input.label,
    subtitle: input.kind === 'collection'
      ? `${count} 个对象 · 原地展开/收起`
      : `${count} 个对象 · 打开 ${noun}`,
    x: input.containerPosition.x,
    y: input.containerPosition.y,
    ...
    contextOnly: input.kind === 'context',
  }
  const scope: CanvasScope = {
    id: scopeId,
    label: input.label,
