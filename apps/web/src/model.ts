...
/** Core Note 锚点（与 packages/domain NoteAnchor 同构，仅供 GUI 定位投影使用）。 */
export type CanvasNoteAnchor =
  | { readonly type: 'project' }
  | { readonly type: 'scope'; readonly scopeId: string }
  | { readonly type: 'artifact'; readonly artifactId: string }
...
export interface CanvasNode {
  ...
  /** GUI-6：Core Note 正文（投影用，非 Artifact 内容）。 */
  noteBody?: string
...
export interface ActiveRun {
  ...
  /** Accepted Agent result identity retained after pending draft is cleared. */
