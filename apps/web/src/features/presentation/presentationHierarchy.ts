...
/** 由 order+depth 推导 parentByViewId（确定性，无环）。 */
export function parentByIdOf(state: PresentationHierarchyState): Record<string, string | null> {
  const parentByViewId: Record<string, string | null> = {}
  const stack: string[] = []
  state.orderIds.forEach((id) => {
    const depth = clampDepth(state.depthById[id] ?? 0)
    while (stack.length > depth) stack.pop()
    parentByViewId[id] = stack.length > 0 ? stack[stack.length - 1]! : null
    stack.push(id)
  })
...
/**
 * Phase 3 §6.6：环守卫。沿 newParent 的祖先链向上，若遇到 childId 则成环。
 */
export function wouldCreateCycle(state: PresentationHierarchyState, childId: string, newParentId: string | null): boolean {
...
/**
 * Phase 3 §6.6：ReparentCommand —— 移动整棵子树到 newParent 之下（beforeSiblingId 存在时插到该兄弟之前）。
 * 校验：目标存在、非自挂、不成环；失败返回 null，绝不部分提交。
 */
export function reparentHierarchyNode(
  state: PresentationHierarchyState,
  childId: string,
  newParentId: string | null,
  beforeSiblingId?: string,
): PresentationHierarchyState | null {
  const childIndex = state.orderIds.indexOf(childId)
  if (childIndex < 0) return null
  if (newParentId !== null && !state.orderIds.includes(newParentId)) return null
  if (wouldCreateCycle(state, childId, newParentId)) return null

  const childDepth = clampDepth(state.depthById[childId] ?? 0)
  let blockEnd = childIndex + 1
  while (blockEnd < state.orderIds.length && clampDepth(state.depthById[state.orderIds[blockEnd]!] ?? 0) > childDepth) blockEnd += 1
  const block = state.orderIds.slice(childIndex, blockEnd)
  const currentParent = parentByIdOf(state)[childId] ?? null
  const targetDepth = newParentId === null ? 0 : clampDepth(state.depthById[newParentId] ?? 0) + 1
  const depthDelta = targetDepth - childDepth
  if (depthDelta === 0 && currentParent === newParentId) {
    // 同父重排：退化为兄弟顺序移动
    if (beforeSiblingId === undefined || beforeSiblingId === childId) return state
    return moveHierarchySubtreeBefore(state, childId, beforeSiblingId)
  }

  const remaining = state.orderIds.filter((id) => !block.includes(id))
  let insertAt = remaining.length
  if (beforeSiblingId !== undefined && remaining.includes(beforeSiblingId)) {
    insertAt = remaining.indexOf(beforeSiblingId)
  } else if (newParentId !== null) {
    const parentIndex = remaining.indexOf(newParentId)
    if (parentIndex >= 0) {
      const parentDepth = clampDepth(state.depthById[newParentId] ?? 0)
      let cursor = parentIndex + 1
      while (cursor < remaining.length && clampDepth(state.depthById[remaining[cursor]!] ?? 0) > parentDepth) cursor += 1
      insertAt = cursor
    }
  }

  const depthById = { ...state.depthById }
  block.forEach((id) => { depthById[id] = clampDepth((depthById[id] ?? 0) + depthDelta) })
  remaining.splice(insertAt, 0, ...block)
  return { ...state, orderIds: remaining, depthById, version: state.version + 1 }
}
...
/** Move a whole hierarchy subtree after another sibling, preserving subtree depth. */
export function moveHierarchySubtreeAfter(state: PresentationHierarchyState, fromId: string, toId: string): PresentationHierarchyState {
  if (fromId === toId) return state
  const fromIndex = state.orderIds.indexOf(fromId)
  const toIndex = state.orderIds.indexOf(toId)
  if (fromIndex < 0 || toIndex < 0) return state
  ...
  const block = state.orderIds.slice(fromIndex, fromEnd)
  const remaining = state.orderIds.filter((id) => !block.includes(id))
  const targetStart = remaining.indexOf(toId)
  if (targetStart < 0) return state
  let targetEnd = targetStart + 1
  while (targetEnd < remaining.length && clampDepth(state.depthById[remaining[targetEnd]!] ?? 0) > toDepth) targetEnd += 1
  remaining.splice(targetEnd, 0, ...block)
  return { ...state, orderIds: remaining, version: state.version + 1 }
}
