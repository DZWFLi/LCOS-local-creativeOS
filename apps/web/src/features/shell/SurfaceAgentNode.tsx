...
export function SurfaceAgentNode({ x, y, contextLabel, seedPrompt, surface, onSubmit, onReadRun, onClose }: {
        ...
        : runState?.status === 'waiting_input'
          ...
          : runState?.status === 'review'
            ...
            : runState
              ? 'Agent 工作中'
              : '局部会话'
  const signalState: LcosSignalState = runState?.status === 'failed'
    ? 'failed'
  ...
  return <aside className={`lcos-surface-agent-node ${runState ? `is-${runState.status}` : ''}`} style={{ left: position.x, top: position.y }} data-native-context-menu="true" aria-label="局部 Agent">
    ...
    >
      <span className="lcos-agent-node-identity"><Grip size={11}/><i className="lcos-agent-node-signal" aria-label="局部 Agent 状态"><SessionGlyph/><LcosSignalGlyph state={signalState}/></i><strong>Agent</strong></span><button type="button" onClick={onClose} aria-label="关闭 Agent"><X size={13}/></button>
    </header>
