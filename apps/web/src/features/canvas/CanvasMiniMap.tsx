...
export function CanvasMiniMap({ nodes, workspaceFrames, camera, setCamera, collapsed, onCollapsedChange, safeInsets }: Props) {
  ...
  return <section className="minimap" data-testid="project-minimap" data-node-count={nodes.length} data-frame-count={workspaceFrames.length}>
    ...
    <div className="map-controls"><button aria-label="缩小画布" onClick={() => setCamera((current) => ({ ...current, zoom: Math.max(.35, current.zoom - .1) }))}><Minus size={13} /></button><span>{Math.round(camera.zoom * 100)}%</span><button aria-label="放大画布" onClick={() => setCamera((current) => ({ ...current, zoom: Math.min(1.8, current.zoom + .1) }))}><Plus size={13} /></button><button className="map-fit-content" aria-label="定位内容" onClick={fitContent}><Maximize2 size={12} /><span>定位内容</span></button></div>
  </section>
}
