...
export function nodeDimensions(kind: CanvasNode['kind'], mode: NodeDisplayMode): { width: number; height: number } {
  // LCOS nodes are working materials first. Standard is deliberately dense;
  // Expanded is reserved for explicit deep-reading / review states.
  if (kind === 'process') return mode === 'compact' ? { width: 174, height: 54 } : mode === 'expanded' ? { width: 246, height: 96 } : { width: 210, height: 66 }
  if (kind === 'decision') return mode === 'compact' ? { width: 184, height: 64 } : mode === 'expanded' ? { width: 260, height: 124 } : { width: 220, height: 88 }
  if (kind === 'context') return mode === 'compact' ? { width: 180, height: 66 } : mode === 'expanded' ? { width: 264, height: 144 } : { width: 220, height: 94 }
  if (kind === 'note') return mode === 'compact' ? { width: 170, height: 60 } : mode === 'expanded' ? { width: 244, height: 122 } : { width: 204, height: 84 }
  return mode === 'compact' ? { width: 178, height: 82 } : mode === 'expanded' ? { width: 292, height: 220 } : { width: 226, height: 148 }
}
...
export interface IncrementalPlacementItem { width: number; height: number }
export interface IncrementalPlacementPoint { x: number; y: number }

function paddedRectsOverlap(a: BoundedNode, b: BoundedNode, gap: number): boolean {
  return !(
    a.x + a.width + gap <= b.x
    || b.x + b.width + gap <= a.x
...
/**
 * Deterministic placement for newly imported material.
 *
 * Existing nodes are read-only obstacles: this function never returns adjusted
 * positions for them. New items keep input order and search the nearest lattice
 * position around the user's drop point. This is the client-side counterpart
 * to the "new things find space; old things never move" rule.
 */
export function placeNewNodesIncrementally(
  existing: readonly BoundedNode[],
  newcomers: readonly IncrementalPlacementItem[],
  origin: IncrementalPlacementPoint,
  gap = 18,
): IncrementalPlacementPoint[] {
  ...
  const occupied: BoundedNode[] = [...existing]
  const result: IncrementalPlacementPoint[] = []
  ...
  newcomers.forEach((item, index) => {
    const preferredOffset = offsets[index] ?? offsets[0]
    const search = [preferredOffset, ...offsets.filter((offset) => offset !== preferredOffset)]
    let chosen = { x: origin.x + preferredOffset.x * stepX, y: origin.y + preferredOffset.y * stepY }
    for (const offset of search) {
      const candidate: BoundedNode = {
