...
export function ConversationContextDialog({ open, projectId, scopeId, workspaceId, client, onClose, onImported, onFocusArtifact, onActivateContextSource, onRequestSectionAnnotation }: {
  ...
  return <div className="modal-backdrop" onPointerDown={(event) => dismissFromBackdrop(event, onClose, busy)}><section className="conversation-context-dialog" role="dialog" aria-label="对话上下文">
    ...
    <div className="conversation-layout">
      ...
      <main>
        {projection && <><div className="conversation-tabs"><button className={tab==='timeline'?'active':''} onClick={() => setTab('timeline')}>对话</button><button className={tab==='outline'?'active':''} onClick={() => setTab('outline')}>大纲</button><button className={tab==='graph'?'active':''} onClick={() => setTab('graph')}>关系</button><button className={tab==='search'?'active':''} onClick={() => setTab('search')}>搜索</button><span title="搜索会自动结合全文与内容相似度，不需要选择搜索方式">{projection.semanticIndex.state === 'ready' ? `内容索引已就绪 · ${projection.semanticIndex.indexedMessages} 条` : '全文索引已就绪'}</span></div>
