...
export function ProjectCreateDialog({ open, onCancel, onBrowseDirectory, onInspectDirectory, onCreate }: Props) {
  ...
  const browse = async () => {
    ...
    try {
      ...
      if (intent === 'open') setInspection(await onInspectDirectory(selectedPath))
    } catch (error) {
  ...
  return <div className="project-create-layer" role="presentation">
    <section className="project-create-dialog" role="dialog" aria-modal="true" aria-labelledby="project-create-title">
      ...
      <div className="project-create-body">
        ...
        {inspection?.requiresConfirmation && <label className="project-root-import-confirm"><input type="checkbox" checked={importExisting} onChange={(event) => setImportExisting(event.target.checked)} /><span><b>把现有内容建立为 Canvas 节点</b><small>{inspection.fileCount} 个文件 · {inspection.directoryCount} 个子文件夹 · {(inspection.totalBytes / 1024 / 1024).toFixed(1)} MB{inspection.skipped.length ? ` · ${inspection.skipped.length} 项将跳过` : ''}</small></span></label>}
        <p>{intent === 'create' ? `Local Core 将在父目录下创建：${label.trim().replace(/\s+/g, '-') || '<项目名>'}` : 'Local Core 只读扫描并登记所选目录；不会移动或改写其中的文件。'}</p>
      </div>
      <footer><button className="secondary" onClick={onCancel}>取消</button><button className="primary" disabled={!label.trim() || !localPath.trim() || browsing} onClick={submit}>{intent === 'create' ? '创建项目' : '打开项目'}</button></footer>
    </section>
  </div>
}
