import { Bot, LayoutGrid, Network, Sparkles, Unplug } from 'lucide-react'
import { useEffect, useMemo, useRef, useState } from 'react'
import type { CSSProperties, PointerEvent as ReactPointerEvent } from 'react'
import type { CanvasEdge, CanvasNode } from '../../model'
import { layoutPreview as runLayoutPreview, layoutPreviewSync } from '../layout/layoutService'
...
export function WorkflowSurface(props:Props){
  ...
  const previewLayout=async ()=>{
    ...
    const engines=await loadPresentationLayoutEngines().catch(()=>({}))
    const result=await runLayoutPreview(request,engines)
  ...
  const overlay=<>
    ...
    {!items.length&&<div className="lcos-workflow-empty"><Network size={19}/><strong>从项目正在做的事开始</strong><span>工作流不会自动吞入整个项目。选择对象，或让 Agent 按当前 Intent 搭建临时 View。Skill 文件放进工作流就是普通可引用对象。</span><div className="lcos-workflow-start-actions"><button type="button" disabled={!props.selectedIds.length} onClick={()=>props.onStart?.('selection')}><Network size={12}/>从 Selection</button><button type="button" onClick={()=>props.onStart?.('agent')}><Bot size={12}/>让 Agent 搭建</button></div></div>}
  </>
  ...
  return <section className="lcos-dedicated-surface lcos-workflow-surface" data-testid="surface-workflow">
    ...
    <details className={`lcos-capability-source source-${props.source?.kind??'empty'}`}>
      ...
      <div><span>Workflow Source</span><small>启发式内容只是建议。Selection 只有显式加入后才成为当前工作流成员。</small><nav><button type="button" disabled={!props.selectedIds.length} onClick={()=>props.onStart?.('selection')}><Network size={11}/>Selection</button><button type="button" onClick={()=>props.onStart?.('agent')}><Bot size={11}/>Agent</button></nav></div>
    </details>
