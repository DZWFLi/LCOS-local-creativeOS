...
export function ContextHistoryRail({ history, handoffs, onBranch, onCompare, onSource }: Props) {
  ...
  return <aside ref={ref} className="lcos-context-history" aria-label="Context history">
    ...
    {handoffs.length > 0 && <div className="lcos-handoff-mini-list">{handoffs.slice(-4).map((handoff) => <div key={handoff.id} className="lcos-handoff-mini"><span>{handoff.from} → {handoff.to}</span><small>{handoff.label || 'Handoff'}</small></div>)}</div>}
    {open && <div className="lcos-context-history-popover" role="dialog" aria-label={`${open.label} history preview`}>
      ...
      <div><button type="button" onClick={() => onSource(open)}><Play size={10}/>来源</button><button type="button" onClick={() => onCompare(open)}><GitCompareArrows size={10}/>对比当前</button><button type="button" className="primary" onClick={() => onBranch(open)}>从这里建现场</button></div>
    </div>}
  </aside>
}
