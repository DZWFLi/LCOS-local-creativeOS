  ...
  return <section className="lcos-dedicated-surface lcos-work-surface" data-testid="surface-work">
    ...
    {showActive&&active&&activeRunNode?<div className="lcos-active-run-stage">
      ...
      <div className="lcos-run-center-stack">
        <button type="button" className={`lcos-run-actor status-${active.status} ${props.selectedIds.includes(activeRunNode.id)?'selected':''}`} onClick={(event)=>props.onSelect(activeRunNode.id,event.shiftKey||event.metaKey||event.ctrlKey)} onDoubleClick={()=>runtime?.onOpenRunDetails(activeRunNode)}><span className="lcos-run-actor-icon"><Play size={15} fill="currentColor"/></span><div><small>{active.provider||activeRunNode.sourceProvider||'Agent 执行'}</small><strong>{activeRunNode.title}</strong><p>{active.command||activeRunNode.commandText||activeRunNode.subtitle}</p></div><span className="lcos-run-actor-status">{runStatusLabel[active.status]}</span>{active.status==='waiting_input'&&<CircleAlert size={14} className="lcos-run-alert"/>}{active.status==='failed'&&<AlertTriangle size={14} className="lcos-run-alert"/>}</button>
    ...
    </div>:<div className="lcos-run-groups">{fallbackGroups.length?fallbackGroups.map(({run,context,output})=><div className="lcos-run-group" key={run.id}><div className="lcos-run-context-list">{context.length?context.map((node)=><SurfaceObject key={node.id} node={node} compact selected={props.selectedIds.includes(node.id)} onSelect={props.onSelect} onDoubleClick={props.onDoubleClick}/>):<span className="lcos-empty-slot">没有额外参考材料</span>}</div><button className={`lcos-run-actor status-${run.runStatus??'idle'} ${props.selectedIds.includes(run.id)?'selected':''}`} onClick={(event)=>props.onSelect(run.id,event.shiftKey||event.metaKey||event.ctrlKey)} onDoubleClick={()=>props.onDoubleClick(run.id)}><span className="lcos-run-actor-icon"><Play size={15} fill="currentColor"/></span><div><small>{run.sourceProvider||'Agent 执行'}</small><strong>{run.title}</strong><p>{run.commandText||run.subtitle}</p></div><span className="lcos-run-actor-status">{run.runStatus?runStatusLabel[run.runStatus]:'执行记录'}</span></button><div className="lcos-run-output-list">{output.length?output.map((node)=><SurfaceObject key={node.id} node={node} compact selected={props.selectedIds.includes(node.id)} onSelect={props.onSelect} onDoubleClick={props.onDoubleClick}/>):<span className="lcos-empty-slot">等待结果</span>}</div></div>):<div className="lcos-work-empty"><Play size={18}/><strong>还没有执行记录</strong><span>在任意工作现场选中内容后，都可以直接开始。</span></div>}</div>}
  </section>
}
