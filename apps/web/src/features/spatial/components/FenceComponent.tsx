...
export function FenceComponent({ element, selected = false, meta }: SurfaceComponentRenderProps) {
  ...
  return <div className={`lcos-surface-fence-body ${selected ? 'is-selected' : ''} ${signal.signalClass}`} data-surface-fence={element.id} data-spatial-signal={signal.state}>
    <LightSegment axis="horizontal" length={48} active={signal.segmentActive}/>
    <div className="lcos-surface-fence-label"><LcosSignalGlyph state={signal.state}/><span><strong>围栏</strong><small>{meta ?? element.presentation?.variant ?? '只组织这里的投影'}</small></span></div>
