...
describe('canvas geometry', () => {
  ...
  it('maps node families to bounded density dimensions', () => {
    expect(nodeDimensions('source', 'compact')).toEqual({ width: 178, height: 82 })
    expect(nodeDimensions('source', 'expanded')).toEqual({ width: 292, height: 220 })
    expect(nodeDimensions('process', 'standard')).toEqual({ width: 210, height: 66 })
  })
