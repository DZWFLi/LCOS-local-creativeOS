...
describe('runEvents — Run 生命周期事件契约', () => {
  it('describe 覆盖全部已知事件类型', () => {
    ...
    expect(describeRunEvent({ type: 'changeset-proposed', changeSetId: 'cs1', title: '改三处' })).toBe('这次修改已准备好：改三处')
    expect(describeRunEvent({ type: 'accept-requested', changeSetId: 'cs1' })).toBe('正在等待你确认这次修改')
    expect(describeRunEvent({ type: 'accept-succeeded', changeSetId: 'cs1' })).toBe('这次修改已确认')
