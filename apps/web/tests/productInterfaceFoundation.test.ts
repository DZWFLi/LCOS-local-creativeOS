...
const drop = readFileSync(new URL('../src/features/drop/DropShelf.tsx', import.meta.url), 'utf8')
...
describe('LCOS product interface foundation', () => {
  ...
  it('keeps primary shell and destination controls accessible by name and state', () => {
    ...
    expect(strip).toContain('aria-label="打开项目 Agent"')
    expect(dock).toContain('aria-label="Scope 与能力入口"')
    expect(dock).toContain('aria-label={`${label}：${hint}`}')
    expect(drop).toContain('role="dialog"')
    expect(drop).toContain('aria-labelledby="drop-shelf-title"')
    expect(drop).toContain('aria-pressed={verb === item}')
