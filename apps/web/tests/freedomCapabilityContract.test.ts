...
describe('VNext.3 capability frame / project semantics contract', () => {
  ...
  it('preserves the interaction hard gates from the hand-tested Silk baseline', () => {
    ...
    expect(canvas).toContain('const edge = 96')
    expect(canvas).toContain('rect.bottom - 96')
    expect(canvas).toContain('rect.left + 96')
    expect(canvas).toContain('restoreDraggedOriginals')
    expect(canvas).toContain('dropStageAnchor.current = anchor')
    expect(canvas).toContain('setDropGutter(null)')
