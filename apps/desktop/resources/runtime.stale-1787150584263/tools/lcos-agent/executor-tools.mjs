...
export const executorToolNames = new Set(["claim_lcos_run","start_lcos_run","heartbeat_lcos_run","fail_lcos_run","get_lcos_task","get_lcos_run_context","request_lcos_user_input","submit_lcos_result","propose_lcos_context_change"]);
...
tool("get_lcos_run_context", "Read the frozen cache-friendly Context prompt for one Run (stable Saved Context prefix + dynamic task tail; never live ActiveContext).", {
    runId: { type: "string" },
  }, ["runId"]),
tool("propose_lcos_context_change", "Create a Context change Proposal for the run's project (S5 Proposal producer). Never applies the change; user accepts/rejects in review.", {
    runId: { type: "string" },
    reason: { type: "string" },
...
export async function invokeExecutorTool(requestedTool, args) {
  ...
  switch (requestedTool) {
    ...
    case "get_lcos_run_context":
      {
        const runId = required(args.runId, "runId");
        value = await coreRequest(`/runs/${encodeURIComponent(runId)}/context-prompt`);
      }
      break;
    ...
    case "propose_lcos_context_change": {
      ...
      const projectId = String(review.run.projectId);
      const active = await coreRequest(`/projects/${encodeURIComponent(projectId)}/active-context`);
      value = await coreRequest(`/projects/${encodeURIComponent(projectId)}/context-proposals`, {
        method: "POST",
        ...jsonBody({
          baseContextVersion: active.version,
          addViewIds: Array.isArray(args.addViewIds) ? args.addViewIds : [],
          removeViewIds: Array.isArray(args.removeViewIds) ? args.removeViewIds : [],
