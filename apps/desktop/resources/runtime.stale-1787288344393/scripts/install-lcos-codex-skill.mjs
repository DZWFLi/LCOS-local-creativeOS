...
const repositoryRoot = resolve(process.env.LCOS_REPO_ROOT || resolve(import.meta.dirname, '..'))
const codexHome = resolve(process.env.CODEX_HOME || join(homedir(), '.codex'))
const skillsRoot = join(codexHome, 'skills')
const managedSkillsManifest = join(repositoryRoot, 'packages', 'skills', 'managed-skills.json')
if (!existsSync(managedSkillsManifest)) {
  throw new Error(`LCOS managed skill manifest is missing: ${managedSkillsManifest}`)
}
const managedSkillsData = JSON.parse(readFileSync(managedSkillsManifest, 'utf8'))
const managedSkills = Array.isArray(managedSkillsData?.skills) ? managedSkillsData.skills : []
if (managedSkillsData?.schemaVersion !== 1 || managedSkills.length === 0 || managedSkills.some((name) => typeof name !== 'string' || !name.trim())) {
  throw new Error(`Invalid LCOS managed skill manifest: ${managedSkillsManifest}`)
}
if (new Set(managedSkills).size !== managedSkills.length) {
  throw new Error(`Duplicate LCOS managed skill in manifest: ${managedSkillsManifest}`)
}

// Utility 完成协议：父进程（runUtilityOnce）以 completion message 判定业务完成，
// 不依赖本进程自然退出（Electron 43 Windows 下 utilityProcess 做完可能不退出）。
// postMessage 后给 stdout 一点 drain 时间再强退；process.exitCode 让自然退出也带正确码。
...
const fail = (message) => { console.error(message); utilityComplete(false, message) }
