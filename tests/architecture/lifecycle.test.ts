...
describe('ARCH-006 Presentation mutations do not bump semanticGraphVersion', () => {
  ...
  it('move/resize/viewport mutations keep graphVersion unchanged', () => {
    ...
    repo.save({ schemaVersion: 4, graphVersion: 1, project: { id: 'p1', name: 'P', rootPath: 'd://p', graphVersion: 1, createdAt: now(), updatedAt: now() }, scopes: [{ id: 's1', projectId: 'p1', parentScopeId: null, containerViewId: null, kind: 'root', name: 'R', createdAt: now(), updatedAt: now() }], workspaces: [{ id: 'w1', projectId: 'p1', scopeId: 's1', name: 'W', intent: null, viewport: { x: 0, y: 0, zoom: 1 }, focusedViewIds: [], visibleLayers: ['core'], contextPolicy: 'selection-only', updatedAt: now() }], artifacts: [{ id: 'a1', projectId: 'p1', title: 'A', kind: 'markdown', availability: 'available', createdAt: now(), updatedAt: now() }], artifactViews: [{ id: 'v1', artifactId: 'a1', scopeId: 's1', referenceKind: 'primary', position: { x: 0, y: 0 }, size: { width: 100, height: 100 }, displayMode: 'card', collapsed: false }], relations: [], notes: [], artifactRevisions: [], fileRecords: [], checkpoints: [] } as any)
    ...
    repo.applyMutations({ baseVersion: v1, ops: [
      ...
      { type: 'resize_artifact_view', viewId: 'v1' as ArtifactView['id'], width: 240, height: 180 },
      { type: 'update_workspace_presentation', workspaceId: 'w1' as any, focusedViewIds: ['v1'] as any, visibleLayers: ['core'] },
    ]})
...
describe('ARCH-007 Semantic mutation bumps version by 1 per batch', () => {
  ...
  it('upsert artifact title bumps version by 1', () => {
    const repo = new SqliteMetadataRepository(join(dir, 'test.sqlite'))
    repo.save({ schemaVersion: 4, graphVersion: 1, project: { id: 'p1', name: 'P', rootPath: 'd://p', graphVersion: 1, createdAt: now(), updatedAt: now() }, scopes: [{ id: 's1', projectId: 'p1', parentScopeId: null, containerViewId: null, kind: 'root', name: 'R', createdAt: now(), updatedAt: now() }], workspaces: [], artifacts: [], artifactViews: [], relations: [], notes: [], artifactRevisions: [], fileRecords: [], checkpoints: [] } as any)
    ...
    const after = repo.get('p1')!
    expect(Number(after.graphVersion)).toBe(Number(before.graphVersion) + 1)
    repo.close()
  })
})
...
describe('ARCH-009 Relation independent from View lifecycle', () => {
  ...
  it('deleting a View keeps the business Relation', () => {
    ...
    repo.save({ schemaVersion: 4, graphVersion: 1, project: { id: 'p1', name: 'P', rootPath: 'd://p', graphVersion: 1, createdAt: now(), updatedAt: now() }, scopes: [{ id: 's1', projectId: 'p1', parentScopeId: null, containerViewId: null, kind: 'root', name: 'R', createdAt: now(), updatedAt: now() }], workspaces: [{ id: 'w1', projectId: 'p1', scopeId: 's1', name: 'W', intent: null, viewport: { x: 0, y: 0, zoom: 1 }, focusedViewIds: [], visibleLayers: ['core'], contextPolicy: 'selection-only', updatedAt: now() }], artifacts: [{ id: 'art-a', projectId: 'p1', title: 'A', kind: 'markdown', availability: 'available', createdAt: now(), updatedAt: now() }, { id: 'art-b', projectId: 'p1', title: 'B', kind: 'markdown', availability: 'available', createdAt: now(), updatedAt: now() }], artifactViews: [{ id: 'v-a', artifactId: 'art-a', scopeId: 's1', referenceKind: 'primary', position: { x: 0, y: 0 }, size: { width: 100, height: 100 }, displayMode: 'card', collapsed: false }], relations: [rel], notes: [], artifactRevisions: [], fileRecords: [], checkpoints: [] } as any)
...
describe('ARCH-011 Fixture data isolation', () => {
  ...
  it('repository rejects writes to non-existing project unless explicitly saved', () => {
    ...
    const note: Note = { id: 'n-bad', projectId: 'never-saved', anchor: { type: 'project' }, body: 'test', createdAt: now(), updatedAt: now() }
