...
3. **Matrix（点阵纹理）**：`MatrixActivity`（`features/spatial/visual/MatrixActivity.tsx`）——八字动词 `gather/spread/gap/flow/pull/break/absorb/emit`，单基色 `--lcos-matrix-dot`，方向 `--matrix-direction`；`density` 夹 `[2,24]`；默认调用仍可由 `active=false` 完全关闭，但 Instrument 可显式 `visible` 让 idle Matrix 以低能量身体存在。verb-flow 动画：`lcos-mx-flow 1.4s linear infinite`，delay `calc(var(--matrix-index) * 110ms)`（css L456）。
