...
const markerCss = read('apps/web/src/spatial-marker.css')
...
check('Beacon is a shared surface-aware Spatial Marker overlay, not Main-only', spatialCanvas.includes('SpatialBeaconLayer') && spatialCanvas.includes('beacon?: SpatialBeaconState | null') && spatialCanvas.includes('surface={spatialMarkerSurfaceForCanvas(testId)}'))
...
check('Beacon exposes its label through the shared Spatial Marker without becoming a card', beacon.includes('<SpatialMarkerLayer') && markerLayer.includes('lcos-spatial-marker-label') && !beacon.includes('Popover') && !beacon.includes('Dialog'))
...
check('Beacon visual is a transient Spatial Marker cursor with Reduced Motion', beacon.includes("attention: 'beacon'") && markerCss.includes('.is-edge-cursor') && markerCss.includes('prefers-reduced-motion'))
