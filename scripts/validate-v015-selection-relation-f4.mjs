...
const checks = [
  ...
  ['A22 Multi Selection restores an explicit aggregate bounding box without turning it into a filled card', /\.selection-bounds \{[\s\S]*?border:1\.25px solid[\s\S]*?background:transparent;/.test(interaction) && interaction.includes('.selection-bounds::before') && interaction.includes('.selection-bounds::after { content:none; }')],
  ['A22 keeps individual selected members visible inside the aggregate Selection bounds', interaction.includes('.canvas-node.multi-selected {') && interaction.includes('outline:1px solid') && interaction.includes('.canvas-node.multi-selected.selected .lcos-object::before { opacity:.52; }') && interaction.includes('.canvas-node.multi-selected.selected .lcos-object::after { opacity:.38; }') && !interaction.includes('opacity:.26;') && !interaction.includes('opacity:.18;')],
