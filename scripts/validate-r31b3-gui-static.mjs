...
const signals = read('apps/web/src/features/spatial/visual/LcosGlyph.tsx')
...
check('Context opens the understanding worksite directly; Graph stays a Lens', dock.includes("if(next === 'context') onSurface('context-space')") && contextSpace.includes('onImportProjectView') && projection.includes("props.surface==='context-graph'?<ContextRelationshipHomeSurface"))
check('Workflow opens the action scene directly; old Graph remains source-compatible', workflowGraph.includes('project action network') && !projection.includes('!props.activeWorkflowId?<WorkflowGraphSurface') && projection.includes("props.surface==='workflow'?<WorkflowSurface"))
...
check('Glyph Micro is reserved for LCOS system action/state language', signals.includes('LcosGlyphState') && signals.includes("'working'") && signals.includes("'waiting'") && signals.includes("'candidate'") && surfaceObject.includes('LcosGlyph'))
