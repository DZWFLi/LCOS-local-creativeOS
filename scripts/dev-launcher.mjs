...
function ownedPidsFromState() {
  ...
  if (state && state.cwd === process.cwd()) {
    pids.push(state.browserPid, ...(Array.isArray(state.browserPids) ? state.browserPids : []), state.launcherPid, state.webPid, state.corePid, state.bridgePid, state.orchestratorPid, state.trayPid)
  }
...
function openBrowserWindow(surface = 'tap') {
  const browser = findBrowser()
  if (!browser) {
    console.log(`Open manually: ${surfaceUrl(surface)}`)
    return null
  }
  mkdirSync(PROFILE_DIR, { recursive: true })
  return spawn(browser, [
    `--app=${surfaceUrl(surface)}`,
    `--user-data-dir=${PROFILE_DIR}`,
    '--no-first-run',
    '--disable-default-apps',
    '--new-window',
    ...(surface === 'companion' ? ['--window-size=520,920', '--window-position=20,60'] : ['--window-size=1440,920', '--window-position=580,60']),
  ], {
    cwd: process.cwd(),
    stdio: 'ignore',
    detached: false,
...
function printStatus() {
  ...
  console.log(`Recorded PIDs: ${state ? JSON.stringify({ launcherPid: state.launcherPid, corePid: state.corePid, webPid: state.webPid, bridgePid: state.bridgePid, orchestratorPid: state.orchestratorPid, browserPid: state.browserPid, browserPids: state.browserPids, trayPid: state.trayPid }) : '(none)'}`)
  console.log('Ports:')
...
async function open() {
  ...
  const surfaces = ['tap', 'companion']
  let openedSurface = false
  for (const surface of surfaces) {
    const browser = openBrowserWindow(surface)
    if (!browser?.pid) continue
    openedSurface = true
    state.browserPids.push(browser.pid)
    if (state.browserPid === null) state.browserPid = browser.pid
    writeState(state)
    console.log(`✓ ${surface === 'tap' ? 'Tap' : 'Companion'} surface opened pid=${browser.pid}`)
    browser.once('exit', () => {
      state.browserPids = state.browserPids.filter((pid) => pid !== browser.pid)
      if (state.browserPid === browser.pid) state.browserPid = state.browserPids[0] ?? null
      writeState(state)
    })
    await new Promise((resolve) => setTimeout(resolve, 450))
  }
  if (openedSurface) {
    console.log('✓ Surface Bootstrap ready: Tap + Companion share the same LCOS runtime. npm run dev:stop stops all.')
  } else {
