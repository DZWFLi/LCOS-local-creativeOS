...
const checks = [
  ...
  ['A16 introduces one shared visible-node -> canonical Relation endpoint resolver', endpoint.includes('projectRelationEndpointForNode') && endpoint.includes('projectRelationEndpointForNodeId') && endpoint.includes("ProjectRelationEndpointType = 'view' | 'note' | 'scope' | 'workspace'")],
  ...
  ['Conversation ordinary Relation remains fail-close in the canonical resolver', endpoint.includes("if (node.entityKind === 'conversation') return null")],
  ...
  ['Unit coverage freezes view, Core Note, aggregate scope/workspace, local-note and Conversation fail-close cases', endpointTest.includes("entityType: 'view'") && endpointTest.includes("entityType: 'note'") && endpointTest.includes("entityType: 'scope'") && endpointTest.includes("entityType: 'workspace'") && endpointTest.includes('toBeNull()')],
