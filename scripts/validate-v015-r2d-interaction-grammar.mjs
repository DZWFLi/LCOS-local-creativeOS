...
const checks = [
  ...
  ['Reference Pick keeps an explicit bridge while Ctrl/Cmd accelerator requires an active Composer', app.includes("referencePick: layoutMode === 'desktop' ? { active: referencePickActive, modifierEnabled: selectionComposerOpen, ids: selectionReferenceIds") && canvas.includes('referencePick.active || (referencePick.modifierEnabled && referenceModifierHeld)') && host.includes('command.referencePickActive || (agentNode && referenceModifierHeld)')],
